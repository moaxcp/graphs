...
----
compile group: 'com.github.moaxcp.graphs', name: 'graphs-core', version: '0.11.0'
----

`graphs-greenrobot` EventGraph implementation using greenrobot's EventBus

----
compile group: 'com.github.moaxcp.graphs', name: 'graphs-greenrobot', version: '0.11.0'
----

`graphs-graphviz` rendering of graph as png or animated gifs using graphviz

----
compile group: 'com.github.moaxcp.graphs', name: 'graphs-graphviz', version: '0.11.0'
----

`graphs-truth` assertions using google-truth

----
compile group: 'com.github.moaxcp.graphs', name: 'graphs-truth', version: '0.11.0'
----
...
* never return null or accept null
...
=== EventGraph
* Works exactly like a `Graph` and sends events to the EventBus when modified
...
There are two other implementations of `Graph` in the 'graphs-greenrobot` module. These implementations send events to the EventBus when the graph is created or modified.
...
An `EventGraph` will send `VertexCreated` events to subscribers.

[source,java,indent=0]
----
include::{sourcedir}/com/github/moaxcp/graphs/manual/RepresentingAGraphAsCode.java[tags=vertexEvent]
----
...
If graph is an `EventGraph` first `VertexCreatedEvent`,then `VertexPropertyEvent` will be sent to the `EventBus`.
...
With an `EventGraph` the `EdgeCreatedEvent` will be sent to the bus. If any of the vertices need to be created `VertexCreatedEvent` will be sent to the bus.

[source,java,indent=0]
----
include::{sourcedir}/com/github/moaxcp/graphs/manual/RepresentingAGraphAsCode.java[tags=edgeEvent]
----
...
The `Graph` interface is used to represent a property graph. This means graphs, vertices, and edges can have properties added to them. These properties can be used to represent
...
The generic parameter in `Graph<ID>` represents the type for all id properties.
...
[source,java,indent=0]
----
include::{sourcedir}/com/github/moaxcp/graphs/manual/VisualizeAGraph.java[tags=dot]
----
...
[source,java,indent=0]
----
include::{sourcedir}/com/github/moaxcp/graphs/manual/VisualizeAGraph.java[tags=gif]
----
...
Adding gif support to graphs-graphviz module. This allows gif animations to be built from an `EventGraph`.
