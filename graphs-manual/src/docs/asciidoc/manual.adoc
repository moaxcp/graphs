...
If a vertex already exists in the graph it will be modified.
...
The first call to `vertex` creates vertex `A`. The second call sets the property `color` to `blue`.

If graph is an `EventGraph` events will be sent to the `EventBus`.
...
If an edge already exits it will be modified.
