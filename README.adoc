...
Javadoc for `Vertex.setId` has been updated to throw IllegalArgumentException of a vertex with id already exists.
Removed EventBus.getBus and generic type.
Fixed bug #22 where setting `from` or `to` on an edge does not result in an event
